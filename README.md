...
## Ship it to the web (Vercel)

The demo goes out as a link, and the link installs. `expo export` produces a static
`dist/` that Vercel serves as-is.

```bash
npm run build:web      # writes dist/
```

**Deploy — connect the repo (recommended, redeploys on push):**

1. Vercel → **Add New… → Project** → import `BetterSaas-engg/Ultimate-Fitness`.
2. Vercel reads `vercel.json`, so the build command (`npx expo export --platform web`)
   and output directory (`dist`) are already set. Don't override them in the dashboard —
   dashboard settings win over the file.
3. **Deploy.** First build takes ~2–3 minutes.
4. **Settings → Domains** to swap the generated URL for something like
   `ultimate-fitness-demo.vercel.app`.

**Deploy — CLI (no repo wiring needed):**

```bash
npm i -g vercel
vercel login
vercel --prod          # builds on Vercel using vercel.json
```

To push a build you made locally instead, `vercel --prod --prebuilt` after `vercel build`.

`dist/` is gitignored — Vercel builds it, you don't commit it.

### What makes it a PWA

- `public/manifest.json` — name, `short_name` (`UF Club`, short enough that Android doesn't
  clip the home-screen label), `display: standalone`, `theme_color: #07A4E7`. Everything in
  `public/` is copied to the root of `dist/`.
- `app/+html.tsx` — the HTML shell for every route: the manifest link, theme colour, and the
  iOS-only meta tags. iOS ignores the manifest's `display` and `icons`, so **Add to Home Screen**
  needs `apple-mobile-web-app-capable` and `apple-touch-icon` separately.
- The page title lives in `app/_layout.tsx` via `expo-router/head`, not in `+html.tsx` —
  expo-router emits its own `<title>` first and the browser takes the first one.
- `vercel.json` — the `/(.*)` → `/index.html` rewrite. Vercel checks the filesystem before
  rewrites, so routes with real HTML files (`/progress`, `/admin/nutrition`, …) are served
  directly; a deep link into a dynamic route like `/admin/workout/p1/1` has no file, falls
  through to the shell, and the router resolves it. Plus immutable cache headers on the
  fingerprinted `/_expo/*` and `/assets/*` bundles.

To install: open the link on a phone, **Share → Add to Home Screen** (iOS) or the install prompt
(Android/Chrome). It launches full-screen with the gym's badge as the icon and no browser chrome.

**Icons** are generated by `npm run icons` from `assets/uf-logo-dark.png`. That source is 291×100 —
the highest resolution the brand has, checked against their site — so the 512px icons are
upscaled ~1.3×. Drop in a larger logo and re-run to sharpen them.
